...
from config import EPOCH, token_size, feature_size, hidden_size, \
    label_size, heads, edge_type, train_prob, val_prob, with_df, cuda_devices, test_mode
...
class GSANN(nn.Module):
    ...
    def forward(self, ast, emb, cfg, label):
        ...
        fea_3 = gat_in_3 + gat_out_3 + fea_2

        pooled, _ = t.max(fea_3, dim=0, keepdim=True)
...
def train():
    ...
    if test_mode:
        model.load_state_dict(t.load('./' + prob + '/' + model_name, map_location=t.device('cpu')))
    model = model.cuda()
    train_datas = load_data(train_prob)
    train_counts = len(train_datas)
    test_datas = load_data(val_prob)

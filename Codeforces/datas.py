...
import torch as th
import dgl
from config import edge_type as n_edge_type, with_df, prob, data_dir
...
def load_data(prob):
    global labels
    # choose codechef experiment
    path = data_dir + prob +'_df.json'
    ...
    for data in tqdm(datas):
        ...
        if n_block <= 2:
            continue
        if len(ast_nodes) >= 100000:
            # too bit for GPU
            continue
        label = data['label']
        if prob not in codechef and prob != 'code':
            label = ''.join(data['label'].split('_')[:-1])
        ...
        data_list.append({
            ...
            'cfg': cfg_graph,
            'emb': emb_graph,
            'label': th.LongTensor([int(label)]).cuda()
        })
        cnt[int(label)] += 1
    print(cnt)
    del datas
